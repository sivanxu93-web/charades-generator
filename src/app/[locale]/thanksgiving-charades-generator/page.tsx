...
const pageKey = "thanksgiving-charades-generator";
...
export default async function ThanksgivingCharadesPage({ params }: PageProps) {
  const { locale: localeParam } = await params;
  const locale = localeParam as Locale;
  const content = stage2Pages[pageKey][locale] ?? stage2Pages[pageKey].en;

  return <CharadesGrowthPage locale={locale} content={content} category="thanksgiving" />;
}
